...
- **FR-023**: When `/speckit.run` is invoked on a feature directory where `spec.md` already exists: presence of `decisions-log.md` written by a prior orchestrator run ⇒ treat as resume (FR-007); absence of `decisions-log.md` ⇒ require an explicit `--force` flag to overwrite, otherwise halt with a clear message.
...
- **SC-001**: A developer can complete a `specify→review→clarify→plan→review→tasks` pipeline run for a well-defined feature (definition: see Assumptions) with no more manual interventions than there are configured BLOCKING gates plus any halts triggered by review findings. (V1 BLOCKING-everywhere posture per ADR-014 means the per-run intervention count is determined by the gate count of the configured target pipeline, not capped at a fixed number; the original ≤3 cap was arithmetically incompatible with FR-008's BLOCKING-everywhere policy.)
...
- **Sidecar format** (ADR-016 follow-up): The `specs/[###]/.run/control-flow.log` sidecar format (JSONL vs. structured markdown) is a plan-phase choice. Either supports the regenerable-cache property; the plan author should match the existing decision-log markdown idiom unless tooling needs argue otherwise.
